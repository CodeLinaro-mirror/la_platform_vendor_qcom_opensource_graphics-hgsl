...
def gen_hgsl_headers(verbose, gen_dir, headers_install, unifdef, hgsl_include_uapi):
    error_count = 0
    for h in hgsl_include_uapi:
        hgsl_uapi_include_prefix = os.path.join(h.split('/include/uapi/')[0],
                                                 'include',
                                                 'uapi') + os.sep

        if not run_headers_install(
                verbose, gen_dir, headers_install, unifdef,
                hgsl_uapi_include_prefix, h): error_count += 1
    return error_count
...
def main():
    ...
    parser.add_argument(
            '--hgsl_include_uapi', required=True, nargs='*',
    ...
    if args.verbose:
        ...
        print('hgsl_include_uapi [%s]' % args.hgsl_include_uapi)
        print('headers_install [%s]' % args.headers_install)
        print('unifdef [%s]' % args.unifdef)

    return gen_hgsl_headers(args.verbose, args.gen_dir,
            args.headers_install, args.unifdef, args.hgsl_include_uapi)
